...
router.post('/', (req, res) => {
  ...
  .then(dbUserData =>
  {
    req.session.save(() =>
    {
      ...
      res.status(200).json(dbUserData).redirect('/');
    });
  })
  .catch(err =>
  {
...
{
  // expects { email: "email@email.dom", password: "pass1234" }
  User.findOne(
  {
  ...
  .then(dbUserData =>
  {
    if (!dbUserData)
    {
      res.status(400).json({ message: 'No user wiht that email address!' });
      return;
    }
    ...
    req.session.save(() =>
    {
      ...
      console.log(dbUserData.username, 'logged into site');
